...
function wpus_admin_code_page() {
	...
	if ($codeID != -1) {
		 ...
		 * Project -> User(s)
		 */
		$sql = "SELECT $wpus->users.id, $wpus->users.realname, $wpus->users.email, " .
			"$wpus->user_projects.code, $wpus->user_projects.ipaddress, " .
			"$wpus->user_projects.`when` FROM $wpus->user_projects LEFT JOIN " .
			"$wpus->users ON $wpus->user_projects.user_id=$wpus->users.id " .
			"WHERE $wpus->user_projects.code='%s'";
		$query = $wpdb->prepare($sql, $code);
		$rows = $wpdb->get_results($query);
		$users = array();
		foreach ($rows as $row) {
		 ...
		 * Code -> Sales
		 */
		$sql = "SELECT $wpus->sales.id, $wpus->sales.realname, " .
			"$wpus->sales.email,$wpus->sales.ordernum FROM $wpus->sales " .
			"INNER JOIN $wpus->sale_codes " .
			"ON $wpus->sales.id=$wpus->sale_codes.sale_id " .
			"WHERE $wpus->sale_codes.code_id=%d";
...
function wpus_admin_sales_page() {
	 ...
	 */
	$sales = array();
	$query = "SELECT id, ordernum, realname, email, entered FROM $wpus->sales";
	$rows = $wpdb->get_results($query);
	foreach ($rows as $row) {
		$s = new WPUS_Sale();

		$vars = array('id', 'ordernum', 'realname', 'email', 'entered');
		foreach ($vars as $v) {
			$s->$v = $row->$v;
		}
	 ...
	 */
	$query = "SELECT $wpus->sale_codes.sale_id, $wpus->projects.name, " .
		"$wpus->codes.code FROM $wpus->sale_codes INNER JOIN " .
		"($wpus->projects INNER JOIN $wpus->codes ON " .
		"$wpus->projects.id=$wpus->codes.project_id) ON " .
		"$wpus->sale_codes.code_id=$wpus->codes.id";
	$rows = $wpdb->get_results($query);
	foreach ($rows as $row) {
		$code = $row->code;
		$href = wpus_admin_url(array('page' => WPUS_PAGES_CODE_PAGE, 'code' => $code));
