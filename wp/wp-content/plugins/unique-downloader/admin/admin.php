...
add_action('admin_notices', 'wpus_admin_notices');

function wpus_admin_notices() {
	global $wpus;
	if ($wpus->HasMessages()) {
		$wpus->DisplayAdminMessages();
	}
}
...
function wpus_handle_post_actions() {
	...
	elseif (isset($_POST['wpus-create-order'])) {
		$required = array('wpus-order-ordernum', 'wpus-order-realname', 'wpus-order-email', 'wpus-order-projects');
		...
		if (!$wpus->HasMessages()) {
			...
			$realname = trim($_POST['wpus-order-realname']);
			...
			if (!$realname) {
				$wpus->AppendMessage("Error: required field real name is empty.");
			}
			if (!$email) {
		...
		if (!$wpus->HasMessages()) {
			...
			if (false === $wpdb->insert($wpus->sales, $fields)) {
				$wpus->AppendMessage("Unable to insert sale into database!");
				$wpus->PersistMessages();
			...
			$codes = array();
			foreach ($projects as $projectID => $count) {
				if (!$count || ($count <= 0)) continue;
				// Load the project and generate a code
				error_log("Generating $count codes for project ID $projectID for sale $ordernum to $realname <$email>");
				$p = wpus_project($projectID);
				$c = $p->GenerateCodes($count);
				...
				foreach ($c as $codeID => $codeCode) {
					...
					if (false === $wpdb->insert($wpus->sale_codes, $fields)) {
						$wpus->AppendMessage("Error inserting sale code into db!");
						$wpus->PersistMessages();
						wp_redirect(wpus_admin_url(array('page' => WPUS_PAGES_SALES_PAGE)));
					}
				}
			...
			$wpus->AppendMessage("Created order $ordernum for $realname &lt;$email&gt;", false);
			$wpus->PersistMessages();
			wp_redirect(wpus_admin_url(array('page' => WPUS_PAGES_SALES_PAGE)));
	...
	elseif (isset($_POST['wpus-order-send'])) {
		...
		if (isset($_POST['wpus-order-id'])) {
			...
			if (!is_numeric($orderID)) {
				...
				$sql = "SELECT ordernum, email, realname FROM $wpus->sales WHERE $wpus->sales.id=%d";
