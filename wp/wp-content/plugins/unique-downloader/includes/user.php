...
function wpus_user($id) {
	...
	$unique_downloader_project_user->firstname = $row->firstname;
	$unique_downloader_project_user->lastname = $row->lastname;
...
function wpus_create_user ($firstname, $lastname, $email, $password) {
	...
	if ($wpdb->insert($wpus->users, $fields) !== false) {
		...
		//echo "<p>In wpus_create_user: done for $email.</p>\n";
		return $wpdb->insert_id;
	}
	
	error_log("A database error occurred while adding a user: " . $wpdb->last_error);
	//echo "<p>Database error while creating user.</p>\n";
...
function wpus_add_project_to_active_user ($projectID, $code = '') {
	die();
}
