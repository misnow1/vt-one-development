...
function wpus_check_post() {
    ...
    /*
     * Check for download link redirect
     */
    $project_id = wpus_get_request_var('item', -1);
    $asset_id = wpus_get_request_var('asset', -1);
    if ($wpus->user && ($project_id != -1) && ($asset_id != -1)) {
        if ($project = wpus_project($project_id)) {
            if ($asset = $project->GetAssetByID($asset_id)) {
                $wpus->LogEvent("User " . $wpus->user->email . " downloading asset $asset_id from project $project_id", WPUS_LOG_CDN_DOWNLOAD);
                $link = $asset->GetSignedDownloadLink();
                wp_redirect($link);
                exit();
            }
        }
	...
	if (!$wpus->user) {
		if (isset($_POST['wpus-login'])) {
			...
			if ($user = wpus_verify_user($email, $password)) {
				...
				$wpus->AppendMessage("Invalid user name or password.  If you've forgotten your password, request a new one using the \"forgot password\" link below.");
		...
		elseif (isset($_POST['wpus-create-account'])) {
			...
			if (!isset($_POST['wpus-realname'])) {
				$wpus->AppendMessage("POST missing required field: user name");
			}
			...
			$realname = trim($_POST['wpus-realname']);

			if (empty($realname)) {
				$wpus->AppendMessage("Your name is required to create an account.");
			}
			if (empty($email)) {
			...
			$userResult = wpus_create_user($realname, $email, $password);
			...
			if (empty($email)) {
				...
				if (!$user) {
					...
					if (wpus_send_reset_mail($user)) {
						$wpus->AppendMessage("A password reset e-mail has been sent to $email. Please check your spam or junk mail folder if you have not received it in a few minutes.", false);
					}
					else {
	...
	if (isset($_POST['wpus-access-code'])) {
		...
		if ($status >= 0) {
			...
			if ($wpus->user) {
				$wpus->LogEvent("User " . $wpus->user->email . " registering code $code.", WPUS_LOG_USER_ADD_CODE);

				// handle the pending project code
				$wpus->SetPendingCode($status, $code);
				if ($wpus->AddPendingProjectToActiveUser()) {
                    $target = wpus_redirect_url();
                    $wpus->LogEvent("User " . $wpus->user->email . " successfully registered code $code, redirecting to $target.", WPUS_LOG_USER_ADD_CODE);
					wp_redirect($target);
					exit();
				}
				else {
			...
			$wpus->LogEvent("Code $code generated error: " . wpus_human_readable_error($status), WPUS_LOG_BAD_CODE);
...
function wpus_unique_downloader_tag_func( $atts ) {
	...
	if (isset($_SESSION['pwresetsuccess'])) {
		$wpus->AppendMessage("Your password was changed successfully. Please log in below.", true);
...
function wpus_register_code($code) {
	...
	if ($codeo = wpus_code_by_code($code)) {
        $wpus->LogEvent("Decrementing remaining uses for code $code", CODE);
		$codeo->remaining_uses--;
		$remaining_uses = $codeo->remaining_uses;
		if (false === $wpdb->update($wpus->codes, array('remaining_uses' => $remaining_uses), array('id' => $codeo->id))) {
            error_log("Failed to update remaining uses for code $code", CODE);
            return false;
        }
	}
