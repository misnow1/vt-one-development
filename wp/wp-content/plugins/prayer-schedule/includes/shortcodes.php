...
class WPPS_ShortcodeManager {
	...
	function do_shortcode_tag( $m ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				if ( $this->shortcode_tags[$tag]['has_name'] && ! empty( $attr['options'] ) ) {
					...
					if ( ! wpps_is_name( $scanned_tag['name'] ) )
			...
			if ( WPPS_USE_PIPE ) {
				$pipes = new WPPS_Pipes( $scanned_tag['raw_values'] );
		...
		$scanned_tag = apply_filters( 'wpps_form_tag', $scanned_tag, $this->exec );
	...
	function shortcode_parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = wpps_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
...
$wpps_shortcode_manager = new WPPS_ShortcodeManager();

function wpps_add_shortcode( $tag, $func, $has_name = false ) {
	global $wpps_shortcode_manager;

	return $wpps_shortcode_manager->add_shortcode( $tag, $func, $has_name );
}

function wpps_remove_shortcode( $tag ) {
	global $wpps_shortcode_manager;

	return $wpps_shortcode_manager->remove_shortcode( $tag );
}

function wpps_do_shortcode( $content ) {
	global $wpps_shortcode_manager;

	return $wpps_shortcode_manager->do_shortcode( $content );
}

function wpps_get_shortcode_regex() {
	global $wpps_shortcode_manager;

	return $wpps_shortcode_manager->get_shortcode_regex();
}
