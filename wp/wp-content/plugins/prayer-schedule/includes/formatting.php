...
function wpps_strip_quote( $text ) {
	$text = trim( $text );
...
function wpps_strip_quote_deep( $arr ) {
	if ( is_string( $arr ) )
		return wpps_strip_quote( $arr );

	if ( is_array( $arr ) ) {
		$result = array();
		foreach ( $arr as $key => $text ) {
			$result[$key] = wpps_strip_quote( $text );
		}
		return $result;
	}
}

function wpps_canonicalize( $text ) {
...
function wpps_sanitize_file_name( $filename ) {
	/* Memo:
	...
	// it in turn calls sanitize_file_name(). Therefore this wpps_sanitize_file_name() will be
